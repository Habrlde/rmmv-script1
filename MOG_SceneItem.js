...
/*:
 * @plugindesc (v1.1)[v1.7]  面板 - 全自定义道具界面
...
//修改：weapon和armor改为item和clue的激活状态位图
this._comImg[2] = ImageManager.loadMenusitem(Moghunter.src_item_menu_com_2);
this._comImg[3] = ImageManager.loadMenusitem(Moghunter.src_item_menu_com_3);
...
Scene_Item.prototype.updateButtons = function() {
	for (var i = 0; i < this._buttons.length; i++) {
		if (this._categoryWindow._index === i && !this._actorWindow.active) {//按钮激活状态
			//插入：位图改变和位置右移20像素
			if (this._buttons[i].activeState !== 'active') {
				this._buttons[i].bitmap = this._comImg[i + 2];
                			this._buttons[i].x = this._buttons[i]._org_x + 20;
               			 this._buttons[i].activeState = 'active';
			}
		    ...
		    this._buttonsAni[i] = 0
			//未激活时，位图和位置复原
			if (this._buttons[i].activeState !== 'inactive') {
                			this._buttons[i].bitmap = this._comImg[i];
                			this._buttons[i].x = this._buttons[i]._org_x;
                			this._buttons[i].activeState = 'inactive';
            			}
...
Scene_Item.prototype.addCatIndex = function(value) {
	 ...
	 if (this._categoryWindow._index > 1) {this._categoryWindow._index = 0};
	 if (this._categoryWindow._index < 0) {this._categoryWindow._index = 1};
